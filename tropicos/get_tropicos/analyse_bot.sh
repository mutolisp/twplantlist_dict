...
## 
# The search results is as following:
# Family        ! Scientific Name                            Author       Reference                                      Date
# Lycopodiaceae ! Lycopodium serratum                        Thunb.       Syst. Veg. (ed. 14) 944                        1784
#### following is the dirty method _D_ 2009.10.22
D_base=`cat ${i}.txt | grep Family`
D_base_line=`cat ${i}.txt | grep -n "Family" | awk -F':' '{print $1}'`
## remove setion(sect.), subgenus(subgen.), divergence(div.) (ser.--> not yet processed, because it's mixed with reference series
# abbreviation (ser.)
if [ `grep " sect. " ${i}.txt | wc -l` -gt 0 ] || [ `grep " div " ${i}.txt | wc -l` -gt 0 ] ||  [ `grep " subgen. " ${i}.txt | wc -l` -gt 0 ] ; then
        D_scbase_line=`cat ${i}.txt | grep -n ${family} | grep -v " subgen. " | grep -v " div "  | grep -v " sect. " | awk 'NR==1' | awk -F':' '{print $1}'`
        accepted="O"
    elif [ `grep "\!" ${i}.txt | wc -l` -gt 2 ]; then 
        # There are two ! in tropicos search, one in header and another on the bottom of table. 
        D_scbase_line=`grep -n "\!" ${i}.txt |  grep -v Family | grep -v Accepted | awk 'NR==1' | awk -F':' '{print $1}'`
        echo "This is accepted by Missouri Botanical Garden"
        accepted="Y"
    elif [ `grep "\!" ${i}.txt | wc -l` -eq 2 ]; then
...
namelist=1_4k
#:> Z_tropicos_names
#namelist=Lycopodium_serratum
output=C2_results
echo "Condition|Accepted|sn|family|tropicos_fam|tropicos name|author|reference|year" > ${output}
...
for s in `cat ${namelist} | awk -F',' '{ print $3}'`
    ...
    ##### FIND RECORDS ######
        ...
        if [ `cat ${i}.txt | grep ! | wc -l` -gt 1 ]; then
              family=`cat ${namelist} | grep ${s} | awk -F',' '{print $2}'`
              find_multiple_records
              echo "${condition}|${accepted}|${i}|${family}|${tropicos_fam}|${tropicos}|${author}|${publish_raw}|${year}"  >> ${output}
              # clear the variables
              tropicos_fam=""
              family=""
        else
...
#        #            author='${author}', publish='', year='' "
    ...
    #echo "${condition}|${i}|${family}|${tropicos_fam}|${tropicos}|${author}|${publish_raw}|${year}" >> ${output}
    let i+=1
done  
